...
class WizardService {
    ...
    static async searchWizardById(id) {
        try {
            ...
            const [wizardRows] = await pool.query('SELECT character_id, mana, maxMana FROM wizards WHERE character_id = ?', [id]);
    ...
    static async getAllWizards() {
        ...
        try {
            ...
            const [wizardsRows] = await connection.query('SELECT character_id, mana, maxMana FROM wizards');
    ...
    static async restoreMana(wizardId) {
        try {
            ...
            await pool.query('UPDATE wizards SET mana = ? WHERE character_id = ?', [wizard.mana, wizard.id]);
